...
const ALLOW_REBUILD = process.env.LOTTIE_ZAP_ALLOW_REBUILD === 'true';

function buildRebuildBlockedMessage(prefix, slug = null) {
    const command = slug ? `${prefix}lotti ${slug} original` : `${prefix}lotti <slug> original`;
    return [
        'A reconstrucao/modificacao de Lottie esta desativada por padrao.',
        'Clientes recentes podem verificar a integridade da animacao e rejeitar stickers reconstruidos.',
        '',
        `Use o relay original com: ${command}`,
        'Se ainda quiser testar rebuild por sua conta e risco, inicie com:',
...
async function sendSavedOriginal(sock, jid, quoted, resolved, baileys, finalSettings) {
    ...
    if (!ALLOW_REBUILD) {
        throw new Error('Relay original indisponivel sem sourceData. Rebuild esta desativado por padrao.');
    }
...
async function execute(sock, messageData, args = [], options = {}) {
    ...
    try {
        ...
        if (!ALLOW_REBUILD && !shouldSendOriginal) {
            return sock.sendMessage(from, {
                text: buildRebuildBlockedMessage(prefix, resolved?.preset?.slug || parsed.presetName || null)
            }, { quoted: quoteThis });
        }

        if (!ALLOW_REBUILD && shouldSendOriginal && (inline.pack || inline.publisher)) {
            return sock.sendMessage(from, {
                text: [
                    'Overrides de pack/publisher exigem rebuild da Lottie salva.',
                    buildRebuildBlockedMessage(prefix, resolved?.preset?.slug || parsed.presetName || null)
                ].join('\n\n')
            }, { quoted: quoteThis });
        }
