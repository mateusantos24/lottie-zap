...
function resolveLottieMedia(messageData = {}) {
    for (const media of [
        messageData.quotedMedia,
        messageData.decryptedMedia,
        messageData.currentMedia
    ]) {
        if (isLottieMedia(media)) return media;
    }
...
async function execute(sock, messageData, args = []) {
    ...
    const media = resolveLottieMedia(messageData);

    if (!isLottieMedia(media)) {
        return sock.sendMessage(from, {
            text: `Responda ou marque uma figurinha Lottie (.was) e use:\n${prefix}addstickerlottier ${title}`
        }, { quoted: quoteThis });
    }
...
module.exports = {
    ...
    aliases: ['addstickerlottie', 'setstickerlottie', 'setstickerlottier', 'addlottier', 'setlottier'],
    execute
};
