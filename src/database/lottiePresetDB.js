...
async function extractAnimationTemplate(slug, wasBuffer) {
    ...
    const secondaryOriginalText = await readZipText(zip, 'animation_secondary.json');
    const animationTrustTokenText = await readZipText(zip, 'animation.json.trust_token');
    const animationSecondaryTrustTokenText = await readZipText(zip, 'animation_secondary.json.trust_token');
    const metadataJsonText = await readZipText(zip, 'animation.json.overridden_metadata', false);
    const imageSlot = forceImageSlotSecondaryText(secondaryOriginalText, slug);
...
async function savePreset({ title, wasBuffer, sourceData = null, actor = null }) {
    ...
    db.prepare(`
        INSERT INTO lottie_presets (
            slug, title, file_size, file_sha256, source_json, was_blob,
            template_animation_json, template_animation_secondary_json,
            template_animation_trust_token, template_animation_secondary_trust_token,
            template_metadata_json, template_native_secondary_json, image_slot_applied,
            created_by, updated_by, created_at, updated_at, usage_count
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
    `).run(slug, ...values.slice(0, -2), actor, actor, now, now);
